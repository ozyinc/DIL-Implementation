...
const collidingSuggestedEventsWithBusyTimes = [{
    $set: {
        study_plan: {_id: "$_id"}
    }
}, {
    ...
    $set: {
        events: {$filter: {input: "$events", as: "event", cond: {$in: ["$$event._id", "$suggested_events"]}}}
    }
}, {
    $unset:
        "suggested_events"
...
const eventCollusionsPerLocation = [{
    $project: {
        location_id: 1,
        timing: 1,
    }
    ...
    $set: {
        timing: {
            $map: {
                input: "$timing", as: "time", in: {
                    start: {
                        $ifNull: ["$$time.start", {
                            $add: ["$$time.week",
                                {$multiply: [{$floor: {$divide: ["$$time.slot", 12]}}, 24, 60, 60, 1000],},
                                {$multiply: [8, 60, 60, 1000]},
                                {$multiply: [{$floor: {$mod: ["$$time.slot", 12]}}, 60, 60, 1000],}]
                        }]
                    },
                    end: {
                        $ifNull: ["$$time.end", {
                            ...
                            $map: {
                                input: {
                                    $filter: {
                                        input: "$times",
                                        as: "other",
                                        cond: {$gt: ["$$other.start", "$$time1.start"]}
                                    }
                                },
                                as: 'time2',
                                in: {
    ...
    $set: {
        collusions: {$filter: {input: "$collusions", as: "collusion", cond: "$$collusion.collides"}}
    }
}, {
    $project: {
        times: 0,
    ...
    $group: {
        _id: "$_id",
        event_pairs: {
            $push: {fst: "$collusions.t1.event_id", snd: "$collusions.t2.event_id"}
        },
        collusion_count: {
            $sum: 1
        }
